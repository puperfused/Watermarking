...
bool checkbit(wstring str,int pos)
{
    if(pos<64)
    {
	return (str[pos]=='1'?true:false);
    }
    return false;
}
		...
		if(ch==' ')
		{
			...
			if(bitpos+3<=64)
			{
				...
				int k=0;
        			...
        			{
					int pow_val=int(watermark[i]-48);
        	       		 	val=val+(pow_val*pow(2,k));
					k++;
				}
				wchar_t temp=homospace[val];
				//wprintf(L"returned from function search_space");
				wprintf(L"%lc",temp);
				bitpos+=3;
				continue;
			}
